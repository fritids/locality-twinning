...
function cimy_manage_upload($input_name, $user_login, $rules, $old_file=false, $delete_file=false, $type="", $new_filename="") {
    ...
    if (($file_error == 0) && (!empty($file_name))) {
        ...
        $status = wp_handle_upload($_FILES[$input_name], array('test_form' => false, 'action' => ' wp_handle_upload'));
        ...
        if(!empty($_POST['user_id'])){
            wheels_avatar_invalidate_cimy($_POST['user_id'],81);
            wheels_avatar_invalidate_cimy($_POST['user_id'],30);
            wheels_avatar_invalidate_cimy($_POST['user_id'],48);
            wheels_avatar_invalidate_cimy($_POST['user_id'],35);
            wheels_avatar_invalidate_cimy($_POST['user_id'],50);
        }
    }
    else
        $data = "";
...
//----------------------------------
function wheels_avatar_invalidate_cimy($ID,$size=81)
{
    $urls = array(
