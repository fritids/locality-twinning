...
function login_header($title = 'Log In', $message = '', $wp_error = '') {
    ...
    ?>
<?php global $pageTitle; $pageTitle = 'Forgot Information' ?>
<?php get_header('meta')?>
    <style type="text/css">
        #login_error{color:red;margin: 10px 0;}
        #pass-strength-result{color:green; margin: 5px 0;}
    </style>
...
<div id="login">
<h1>
    <a href="<?php echo esc_url( apply_filters('', '') ); ?>" title="<?php echo esc_attr( apply_filters('', __( '' ) ) ); ?>"></a>
</h1>
        <?php   } else { ?>
<div id="login"><h1><a href="<?php echo esc_url( apply_filters('', network_home_url() ) ); ?>" title="<?php echo esc_attr( apply_filters('', $current_site->site_name ) ); ?>"><span class="hide"></span></a></h1>
        <?php   } ?>
    <h3>Forgot Information</h3>
    <?php
    ...
    if ( $wp_error->get_error_code() ) {
        ...
        if ( !empty($errors) )
            echo '<div id="login_error" class="err-msg-ttl">' . apply_filters('login_errors', $errors) . "</div>\n";
        if ( !empty($messages) )
            echo '<p class="message scs-msg-ttl">' . apply_filters('login_messages', $messages) . "</p>\n";
    }
} // End of login_header()
...
switch ($action) {
    ...
    case 'logout' :
        ...
        $redirect_to = !empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'lostpassword?loggedout=true';
    ...
    case 'retrievepassword' :
        ...
        if ( $http_post ) {
            ...
            if ( !is_wp_error($errors) ) {
                $redirect_to = !empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'lostpassword?checkemail=confirm';
                wp_safe_redirect( $redirect_to );
                exit();
            }
        }
        ...
        <div class="profile-form">
            <form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( site_url( 'lostpassword?action=lostpassword', 'login_post' ) ); ?>" method="post">
                ...
                <input type="submit" name="wp-submit" id="wp-submit" class="formbtn green" value="<?php esc_attr_e('Get New Password'); ?>" tabindex="100" />
            </form>
        </div>
    ...
    case 'rp' :
        ...
        if ( is_wp_error($user) ) {
            wp_redirect( site_url('lostpassword?action=lostpassword&error=invalidkey') );
            exit;
        }
        ...
        if ( isset($_POST['pass1']) && $_POST['pass1'] != $_POST['pass2'] ) {
            ...
            login_header( __( 'Password Reset' ), '<p class="message reset-pass">' . __( 'Your password has been reset.' ) . '</a></p>' );
            //login_footer();
            ?>
            </div>
            </div>
        ...
        <div class="profile-form">
            ...
            <form name="resetpassform" id="resetpassform" action="<?php echo esc_url( site_url( 'lostpassword?action=resetpass&key=' . urlencode( $_GET['key'] ) . '&login=' . urlencode( $_GET['login'] ), 'login_post' ) ); ?>" method="post">
                ...
                <br class="clear" />
                <input type="submit" name="wp-submit" id="wp-submit" class="formbtn green" value="<?php esc_attr_e('Reset Password'); ?>" tabindex="100" />
            </form>
    ...
    case 'register' :
        ...
        if ( !get_option('users_can_register') ) {
            wp_redirect( site_url('lostpassword?registration=disabled') );
            exit();
        }
        ...
        if ( $http_post ) {
            ...
            if ( !is_wp_error($errors) ) {
                $redirect_to = !empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : 'lostpassword?checkemail=registered';
                wp_safe_redirect( $redirect_to );
                exit();
            }
        }
        ...
        <form name="registerform" id="registerform" action="<?php echo esc_url( site_url('lostpassword?action=register', 'login_post') ); ?>" method="post">
            <p>
...
//        $rememberme = ! empty( $_POST['rememberme'] );
        ...
        <p id="nav">
            ...
            <a href="<?php echo esc_url( site_url( 'lostpassword?action=register', 'login' ) ); ?>"><?php _e( 'Register' ); ?></a> |
