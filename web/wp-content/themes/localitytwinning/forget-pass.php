...
function retrieve_password() {
	...
	$message .= '' . network_site_url("forget-pass?action=rp&key=$key&login=" . rawurlencode($user_login), 'login') . "\r\n";
	...
	if ( $message && !wp_mail($user_email, $title, $message) ){
		...
		if(local_send_mail($user_email, $title, $message)){
...
case 'retrievepassword' :
	...
	if ( $http_post ) {
		...
		if ( !is_wp_error($errors) ) {
			$redirect_to = !empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'forget-pass?checkemail=confirm';
			wp_safe_redirect( $redirect_to );
			exit();
		}
	}
...
<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( site_url( 'forget-pass?action=lostpassword', 'login_post' ) ); ?>" method="post">
	<p>
...
<?php endif; ?>
            ...
            case 'rp' :
                ...
                if ( isset($_POST['pass1']) && $_POST['pass1'] != $_POST['pass2'] ) {
                    ...
                    login_header( __( 'Password Reset' ), '<p class="message reset-pass">' . __( 'Your password has been reset.' ) . ' <a href="' . esc_url( site_url('sign-in') ) . '">' . __( 'Log in' ) . '</a></p>' );
                    login_footer();
                    exit;
                }
            ...
            ?>
<form name="resetpassform" id="resetpassform" action="<?php echo esc_url( site_url( 'forget-pass?action=resetpass&key=' . urlencode( $_GET['key'] ) . '&login=' . urlencode( $_GET['login'] ), 'login_post' ) ); ?>" method="post">
...
<p id="nav">
<a href="<?php echo esc_url( site_url('sign-in')); ?>"><?php _e( 'Log in' ); ?></a>
<?php if ( get_option( 'users_can_register' ) ) : ?>
 | <a href="<?php echo esc_url( site_url( 'wp-login.php?action=register', 'login' ) ); ?>"><?php _e( 'Register' ); ?></a>
<?php endif; ?>
</p>
...
<a href="<?php echo esc_url( site_url( 'sign-in?action=register', 'login' ) ); ?>"><?php _e( 'Register' ); ?></a> |
<a href="<?php echo esc_url( wp_lostpassword_url() ); ?>" title="<?php esc_attr_e( 'Password Lost and Found' ); ?>"><?php _e( 'Lost your password?' ); ?></a>
<?php else : ?>
<a href="<?php echo esc_url( wp_lostpassword_url() ); ?>" title="<?php esc_attr_e( 'Password Lost and Found' ); ?>"><?php _e( 'Lost your password?' ); ?></a>
<?php endif; ?>
